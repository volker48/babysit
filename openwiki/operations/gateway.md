...
| A delivery returns 202 but the waiter does not react | Confirm `babysit gateway-token status`, the exact `wss://babysit.mindgoblin.pw/watch` base URL, and the GitHub repository/PR used by the wait. A delivery is only a wake; inspect the subsequent authoritative `gh pr view` fetch and `gh` authentication. |
